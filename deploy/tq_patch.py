...
if '\ubcf5\ud569: \uc2e0\uc120\ucc98\ub7fc \uae30\uc0ac\ubcc4 \ub77c\uc6b0\ud2b8 \ube44\uc728' in s:
    ...
    c=s.count(OLD)
    if c!=1:
        print('OLD_BLOCK_NOT_FOUND count=%d ABORT'%c); sys.exit(1)
    ...
    shutil.copy(F,bak); shutil.move(tmp,F)
    print('PATCH_OK backup='+bak)
u=subprocess.run(['bash','-lc',"systemctl list-units --type=service --no-legend 2>/dev/null | grep -iE 'teamflex|poll|tf-' || true"],capture_output=True,text=True)
print('UNITS:'); print(u.stdout.strip())
